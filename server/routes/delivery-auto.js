...
router.post('/:orderId', auth, async (req, res) => {
  ...
  const providers = db.getDeliveryProviders(req.user.id).filter(p => p.enabled && p.apiType === 'url-recipe');
  ...
  try {
    ...
    db.addLog({ userId: req.user.id, user: 'System', action: `URL automation delivery: ${order.id}`, details: tracking, type: 'delivery', severity: 'success' });
    ...
    db.updateOrder(order.id, { status: 'processing', trackingNumber: tracking, deliveryProvider: prov.name });
    db.addLog({ userId: req.user.id, user: 'System', action: `Manual delivery assist: ${order.id}`, details: `Open ${recipe.createOrderUrl || prov.websiteUrl}`, type: 'delivery', severity: 'info' });
